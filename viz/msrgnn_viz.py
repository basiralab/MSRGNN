...
class SharedGNNReasoner(nn.Module):
    ...
    def __init__(self, fdim_dict, pos_emb_dim, gnn_hidden_dim, 
        ...
        # This classifier is the target predictor/scorer, we keep this name due to legacy reasons
        # as experiments and saved weights refer to it as "classifier".
...
class MSRGNN(nn.Module):
    ...
    The graph structure is fixed at initialisation. You can either specify a
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs/RAVEN_MSRGNN')
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), "./saved_models/best_model_msrgnn.pth")
    ...
    torch.save(model.state_dict(), "./saved_models/model_final_msrgnn.pth")
    print("Training complete. Model saved as 'model_final_msrgnn.pth'.")
    ...
    model.load_state_dict(torch.load("./saved_models/best_model_msrgnn.pth"))
    model.eval()
