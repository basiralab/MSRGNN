...
class StandardGNNBlock(nn.Module):
    """
    A wrapper for standard PyG GNN layers (GCN, GAT) that includes
    a residual connection, normalization, and activation. This creates
    a block structure comparable to the custom GatedGNNRelationLayerV2.
    """
    def __init__(self, gnn_type, in_channels, out_channels, heads=1, dropout=0.1):
        super().__init__()
...
class AblationGNNReasoner(nn.Module):
    """
    An ablation version of the SharedGNNReasoner that uses standard GNN layers
    like GCN or GAT instead of the custom GatedGNNRelationLayerV2.

    This version concatenates visual features and positional embeddings before
    passing them to the GNN layers.
    """
    def __init__(self, fdim_dict, pos_emb_dim, gnn_hidden_dim, gnn_type,
                 gnn_heads, reasoner_proj_dropout):
        super().__init__()
        ...
        # Stage 1: GNN processes each scale independently
        stage1_in_dim = common_gnn_dim + pos_emb_dim
        self.shared_gnn_stage1 = StandardGNNBlock(
            gnn_type=gnn_type,
            in_channels=stage1_in_dim,
            out_channels=gnn_hidden_dim,
            heads=gnn_heads
        )

        # Stage 2: GNN reasons over the fused multi-scale features
        unified_input_dim = len(fdim_dict) * gnn_hidden_dim + pos_emb_dim
        unified_output_dim = gnn_hidden_dim * 2
        self.gnn_stage2 = StandardGNNBlock(
            gnn_type=gnn_type,
            in_channels=unified_input_dim,
    ...
    def forward(self, node_features_dict, batch_indices, edge_index):
        ...
        # --- Stage 1: Independent Scale Processing ---
        stage1_outputs = []
        for name, proj_layer in self.input_projections.items():
            ...
            # Concatenate visual features and positional embeddings for standard layers
            gnn_input = torch.cat([projected_features, pos_embs], dim=1)
            
            processed_features = self.shared_gnn_stage1(gnn_input, edge_index)
            stage1_outputs.append(processed_features)

        # --- Stage 2: Unified Reasoning ---
        unified_h1 = torch.cat(stage1_outputs, dim=1)
        
        # Add positional embeddings again before the second stage
        stage2_input = torch.cat([unified_h1, pos_embs], dim=1)
        
        h2 = self.gnn_stage2(stage2_input, edge_index)
...
class MSRGNN(nn.Module):
    ...
    def __init__(self, ctx_size=8, cand_size=8, resnet_base_channels=64,
        ...
        # Register the chosen template as a buffer to ensure it's handled correctly
        # (e.g., moved to device with .to(device), saved with state_dict)
        ...
        # )
        self.reasoner = AblationGNNReasoner(
                fdim_dict=self.extractor.fdim_dict,
                pos_emb_dim=pos_embedding_dim,
                gnn_hidden_dim=gnn_hidden_dim,
                gnn_type='gat',
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs/RAVEN_MSRGNN_GAT')
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), "./saved_models/best_model_msrgnn_gat.pth")
    ...
    torch.save(model.state_dict(), "./saved_models/model_final_msrgnn_gat.pth")
    print("Training complete. Model saved as 'model_final_msrgnn_gat.pth'.")
    ...
    model.load_state_dict(torch.load("./saved_models/best_model_msrgnn_gat.pth"))
    model.eval()
