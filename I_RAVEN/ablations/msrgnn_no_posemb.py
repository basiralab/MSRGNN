...
class GNNLayer_NoPE(MessagePassing):
    """
    ABLATION GNN LAYER: Operates without any positional embeddings.

    This layer is a direct modification of GatedGNNRelationLayerV2, with all
    positional embedding inputs and logic removed. It relies purely on the
    node's visual features and the graph structure.
    """
    def __init__(self, feature_dim, out_channels,
                 relation_net_mlp_layers, relation_net_dropout, gnn_message_dropout_p):
        super().__init__(aggr='add', node_dim=0)
        print(f"Instantiated GNN Layer without Positional Embeddings (feature_dim: {feature_dim})")

        # --- Key Changes for No PE ---
        # 1. The pairwise function now operates on pairs of node features directly.
        self.pairwise_fn = RelationNetPairwiseV2(
            feature_dim, out_channels, relation_net_mlp_layers, relation_net_dropout
        )

        # 2. Attention is also computed only from visual features.
        self.attention_mlp = nn.Sequential(
            nn.Linear(2 * feature_dim, out_channels),
            nn.LeakyReLU(),
            nn.Linear(out_channels, 1)
        )

        # 3. The update function's input dimension changes.
        self.update_mlp = nn.Sequential(
            nn.Linear(feature_dim + out_channels, out_channels),
            nn.GELU()
        )

        self.norm = LayerNorm(out_channels)
        self.skip_proj = nn.Linear(feature_dim, out_channels) if feature_dim != out_channels else nn.Identity()
        self.gnn_message_dropout_p = gnn_message_dropout_p

    def forward(self, x, edge_index):
        # --- Key Change: pos_emb is no longer an argument ---
        # The 'integrated_x' is just the feature vector 'x'.
        aggr_out = self.propagate(edge_index, x=x)
        # Update step also only uses the node features.
        updated_x = self.update_mlp(torch.cat([x, aggr_out], dim=-1))
        return self.norm(self.skip_proj(x) + updated_x)

    def message(self, x_i, x_j, edge_index_i):
        # --- Key Change: All pos_emb arguments are removed ---
        # The pairwise function now receives node features directly.
        relational_message = self.pairwise_fn(x_j, x_i)
...
class SharedGNNReasoner_NoPE(nn.Module):
    """
    ABLATION REASONER: A two-stage GNN reasoner that does not use any
    positional embeddings. It instantiates and uses the GNNLayer_NoPE.
    """
    def __init__(self, fdim_dict, gnn_hidden_dim,
                 relation_net_mlp_layers, relation_net_dropout,
                 gnn_message_dropout_p, reasoner_proj_dropout):
        super().__init__()
        print("Instantiated Reasoner without Positional Embeddings.")

        # del self.panel_pos_emb
        ...
        # --- Key Change: Instantiate the GNNLayer_NoPE ---
        self.shared_gnn_stage1 = GNNLayer_NoPE(
            feature_dim=common_gnn_dim,
        ...
        unified_input_dim = len(fdim_dict) * gnn_hidden_dim
        unified_output_dim = gnn_hidden_dim * 2
        self.gnn_stage2 = GNNLayer_NoPE(
            feature_dim=unified_input_dim,
            out_channels=unified_output_dim,
            **gnn_params
        )
    ...
    def forward(self, node_features_dict, batch_indices, edge_index, num_nodes_per_graph=9):
        ...
        for name, proj_layer in self.input_projections.items():
            features = node_features_dict[name]
            projected_features = proj_layer(features)
            # GNN layer is called without pos_embs
            processed_features = self.shared_gnn_stage1(projected_features, edge_index)
            stage1_outputs.append(processed_features)

        unified_h1 = torch.cat(stage1_outputs, dim=1)
        # Stage 2 GNN is also called without pos_embs
        h2 = self.gnn_stage2(unified_h1, edge_index)
...
class MSRGNN(nn.Module):
    ...
    def __init__(self, ctx_size=8, cand_size=8, resnet_base_channels=64,
        ...
        self.reasoner = SharedGNNReasoner_NoPE(
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs/RAVEN_MSRGNN_NO_PE')
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), "./saved_models/best_model_msrgnn_no_pe.pth")
    ...
    torch.save(model.state_dict(), "./saved_models/model_final_msrgnn_no_pe.pth")
    print("Training complete. Model saved as 'model_final_msrgnn_no_pe.pth'.")
    ...
    model.load_state_dict(torch.load("./saved_models/best_model_msrgnn_no_pe.pth"))
    model.eval()
