...
class SharedGNNReasoner(nn.Module):
    ...
    def forward(self, node_features_dict, batch_indices, edge_index, num_nodes_per_graph=9):
        ...
        candidate_node_indices = torch.arange(
            start=num_nodes_per_graph - 1,
            end=num_graphs * num_nodes_per_graph,
            step=num_nodes_per_graph,
            device=h2.device
        )
        
        # Select the features of only the candidate nodes
        candidate_features = h2[candidate_node_indices]

        return self.classifier(candidate_features)
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs/RAVEN_MSRGNN_NODE_POOL')
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), "./saved_models/best_model_msrgnn_node_pool.pth")
    ...
    torch.save(model.state_dict(), "./saved_models/model_final_msrgnn_node_pool.pth")
    print("Training complete. Model saved as 'model_final_msrgnn_node_pool.pth'.")
    ...
    model.load_state_dict(torch.load("./saved_models/best_model_msrgnn_node_pool.pth"))
    model.eval()
