...
class SingleScaleResNetExtractor(ResNetFeatureExtractor):
    """
    ABLATION EXTRACTOR: Produces a single feature vector per image panel.

    This extractor uses the same ResNet backbone but replaces the multi-pool
    output stage with a single adaptive pooling layer. This is used to test
    the hypothesis that multi-scale features are critical for performance.
    """
    def __init__(self, in_ch=1, base_channels=64, pool_size=4):
        # We call the parent __init__ to build the ResNet layers.
        super().__init__(in_ch, base_channels)
        print(f"Instantiated Single-Scale Extractor (pool size: {pool_size}x{pool_size})")

        # --- Key Difference: Only use ONE pool and normalizer ---
        final_conv_channels = base_channels * 4
        self.final_pool = nn.AdaptiveAvgPool2d(pool_size)
        dim = final_conv_channels * pool_size * pool_size
        self.final_norm = nn.LayerNorm(dim)
        
        # This is the single feature dimension for this extractor.
        self.fdim = dim

        # We no longer need the multi-scale ModuleDicts from the parent class.
        # Deleting them makes the model cleaner and prevents confusion.
        del self.pools
        del self.normalizers
        del self.fdim_dict

    def forward(self, x):
        # The ResNet backbone forward pass is the same as the parent.
        out = self.gelu(self.bn1(self.conv1(x)))
        out = self.maxpool(out)
        out = self.layer1(out)
        out = self.layer2(out)
        ...
        # --- Key Difference: Process and return a single tensor ---
        pooled = self.final_pool(out).view(x.size(0), -1)
        normalized = self.final_norm(pooled)
        # Returns a tensor of shape (B*P, fdim), not a dictionary.
        return normalized
...
class GatedGNNRelationLayerV2(MessagePassing):
    ...
    def __init__(self, feature_dim, pos_dim, out_channels,
        ...
        # Learns the message content from combined features
        self.pairwise_fn = RelationNetPairwiseV2(
            integrated_dim, out_channels, relation_net_mlp_layers, relation_net_dropout
        )
        
        # Learns attention weights from separate features to preserve signals
...
class SharedGNNReasoner(nn.Module):
    ...
    def __init__(self, fdim_dict, pos_emb_dim, gnn_hidden_dim, 
        ...
        super().__init__()
        self.panel_pos_emb = nn.Embedding(9, pos_emb_dim)
...
class SingleScaleGNNReasoner(nn.Module):
    """
    ABLATION REASONER: Designed to work with a single-scale feature extractor.

    This reasoner processes a single tensor of node features instead of a
    dictionary. It maintains the two-stage GNN architecture to isolate the
    effect of removing the multi-scale inputs.
    """
    def __init__(self, fdim, pos_emb_dim, gnn_hidden_dim,
                 relation_net_mlp_layers, relation_net_dropout,
                 gnn_message_dropout_p, reasoner_proj_dropout):
        super().__init__()
        print("Instantiated Single-Scale GNN Reasoner.")
        self.panel_pos_emb = nn.Embedding(9, pos_emb_dim)
        ...
        # Project from the single feature dimension to the GNN dimension.
        common_gnn_dim = gnn_hidden_dim
        self.input_projection = nn.Linear(fdim, common_gnn_dim)

        # GNN Stage 1: Processes the projected features.
        self.gnn_stage1 = GatedGNNRelationLayerV2(
            common_gnn_dim, pos_emb_dim, gnn_hidden_dim, **gnn_params
        )

        # GNN Stage 2: Processes the output of stage 1.
        stage1_output_dim = gnn_hidden_dim
        stage2_output_dim = gnn_hidden_dim * 2
        self.gnn_stage2 = GatedGNNRelationLayerV2(
            stage1_output_dim, pos_emb_dim, stage2_output_dim, **gnn_params
        )

        # Classifier input is based on the pooled output of stage 2.
        classifier_input_dim = stage2_output_dim * 3 # for mean/max/add pooling
        self.classifier = nn.Sequential(
            nn.Linear(classifier_input_dim, gnn_hidden_dim),
            nn.GELU(),
            nn.Dropout(reasoner_proj_dropout),
    ...
    def forward(self, node_features, batch_indices, edge_index, num_nodes_per_graph=9):
        # This forward pass now accepts a single tensor `node_features`.
        num_graphs = batch_indices.max().item() + 1
        device = batch_indices.device
        ...
        # Project, then run through the two GNN stages.
        projected_features = self.input_projection(node_features)
        h1 = self.gnn_stage1(projected_features, pos_embs, edge_index)
        h2 = self.gnn_stage2(h1, pos_embs, edge_index)
...
class MSRGNN(nn.Module):
    ...
    def __init__(self, ctx_size=8, cand_size=8, resnet_base_channels=64,
        ...
        self.extractor = SingleScaleResNetExtractor(in_ch=1, base_channels=resnet_base_channels, pool_size=1)
        self.ctx_size, self.cand_size = ctx_size, cand_size

        self.reasoner = SingleScaleGNNReasoner(
            fdim=self.extractor.fdim,
    ...
    def forward(self, x, ctx_size, cand_size):
        ...
        extracted_features = self.extractor(x.view(B * P_total, C_in, H, W))
        num_graphs = B * self.cand_size
        num_nodes_per_graph = self.ctx_size + 1

        fdim = self.extractor.fdim
        structured_features = extracted_features.view(B, P_total, fdim)
        ctx_features = structured_features[:, :self.ctx_size, :]
        cand_features = structured_features[:, self.ctx_size:, :]
        
        all_graphs_nodes_list = []
        for i in range(self.cand_size):
            candidate_i = cand_features[:, i, :].unsqueeze(1)
            graph_nodes = torch.cat([ctx_features, candidate_i], dim=1)
            all_graphs_nodes_list.append(graph_nodes)
        
        stacked_graphs = torch.stack(all_graphs_nodes_list).permute(1, 0, 2, 3)
        flat_nodes = stacked_graphs.reshape(num_graphs, num_nodes_per_graph, fdim).view(-1, fdim)
        reasoner_input = flat_nodes
        ...
        edge_index = batch_template_edge_index(
            self.template_edge_index, num_graphs, num_nodes_per_graph, x.device
        )
        
        # 4. Relational Reasoning
        scores = self.reasoner(reasoner_input, batch_indices, edge_index)
...
def main():
    ...
    EPOCHS = 100 
    IMG_SIZE = 80
    CTX_SIZE = 8 # Number of context panels (usually 8 for 3x3 RPM)
    CAND_SIZE = 8 # Number of candidate panels

    PATIENCE = 10


    writer = SummaryWriter(log_dir='runs/RAVEN_MSRGNN_SINGLE_SCALE_1x1')
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), "./saved_models/best_model_msrgnn_single_scale_1x1.pth")
    ...
    torch.save(model.state_dict(), "./saved_models/model_final_msrgnn_single_scale_1x1.pth")
    print("Training complete. Model saved as 'model_final_msrgnn_single_scale_1x1.pth'.")
    ...
    model.load_state_dict(torch.load("./saved_models/best_model_msrgnn_single_scale_1x1.pth"))
    model.eval()
