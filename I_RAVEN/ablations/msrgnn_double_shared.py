...
class MultiScaleTwoHopReasoner(nn.Module):
    """
    ABLATION: Two-Hop Reasoning on Individual Scales without a Unified GNN Stage.

    This reasoner architecture tests the effectiveness of deep, independent reasoning
    at each feature scale.
    - It creates a separate GNN layer for each feature scale.
    - Each GNN layer is applied TWICE (two-hop/recurrently) to its corresponding scale's features.
    - The final node representations from all scales are then concatenated.
    - This combined representation is pooled and passed directly to a classifier,
      skipping the unified GNN stage entirely.
    """
    def __init__(self, fdim_dict, pos_emb_dim, gnn_hidden_dim,
                 relation_net_mlp_layers, relation_net_dropout,
                 gnn_message_dropout_p, reasoner_proj_dropout):
        super().__init__()
        print("Instantiated Reasoner: Multi-Scale Two-Hop (No Unified Stage)")
        self.panel_pos_emb = nn.Embedding(9, pos_emb_dim)
        ...
        # --- Stage 1 Setup ---
        self.input_projections = nn.ModuleDict()
        self.individual_gnns = nn.ModuleDict()
        common_gnn_dim = gnn_hidden_dim

        for name, fdim in fdim_dict.items():
            # Project each scale's features to a common dimension
            self.input_projections[name] = nn.Linear(fdim, common_gnn_dim)
            # Create a dedicated GNN layer for each scale
            self.individual_gnns[name] = GatedGNNRelationLayerV2(
                common_gnn_dim, pos_emb_dim, gnn_hidden_dim, **gnn_params
            )

        # --- Classifier Setup ---
        # The classifier input comes from concatenating the outputs of all
        # individual GNNs, which is then pooled.
        num_scales = len(fdim_dict)
        classifier_input_dim = num_scales * gnn_hidden_dim * 3  # (mean+max+add pooling)
    ...
    def forward(self, node_features_dict, batch_indices, edge_index, num_nodes_per_graph=9):
        ...
        # --- Stage 1: Independent Two-Hop Reasoning per Scale ---
        final_scale_outputs = []
        for name, proj_layer in self.input_projections.items():
            # 1. Get features for the current scale
            features = node_features_dict[name]
            projected_features = proj_layer(features)

            # 2. Get the dedicated GNN for this scale
            gnn = self.individual_gnns[name]

            # 3. Apply the GNN twice (two-hop reasoning)
            h_hop1 = gnn(projected_features, pos_embs, edge_index)
            h_hop2 = gnn(h_hop1, pos_embs, edge_index) # Use output of hop1 as input

            final_scale_outputs.append(h_hop2)

        # --- Final Combination and Classification ---
        # 1. Concatenate final node embeddings from all scales
        final_node_embeddings = torch.cat(final_scale_outputs, dim=1)

        # 2. Pool the combined embeddings across all nodes in each graph
        pooled = torch.cat([
            global_mean_pool(final_node_embeddings, batch_indices),
            global_max_pool(final_node_embeddings, batch_indices),
            global_add_pool(final_node_embeddings, batch_indices)
        ], dim=-1)

        # 3. Classify the final graph representation
        return self.classifier(pooled)
...
class MSRGNN(nn.Module):
    ...
    def __init__(self, ctx_size=8, cand_size=8, resnet_base_channels=64,
        ...
        self.reasoner = MultiScaleTwoHopReasoner(
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs/RAVEN_MSRGNN_DOUBLE_SHARED')
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), "./saved_models/best_model_msrgnn_double_shared.pth")
    ...
    torch.save(model.state_dict(), "./saved_models/model_final_msrgnn_double_shared.pth")
    print("Training complete. Model saved as 'model_final_msrgnn_double_shared.pth'.")
    ...
    model.load_state_dict(torch.load("./saved_models/best_model_msrgnn_double_shared.pth"))
    model.eval()
