from data_utility import DeepiqDataset as dataset
...
class OddOneOutMSRGNN(MSRGNN):
    """
    Adapts MSRGNN to solve the Odd-One-Out (OOO) task using the "group cohesion"
    method by overriding its forward pass.

    This model creates P distinct graphs for a P-panel problem. Each graph 'i'
    omits panel 'i' and includes the other P-1 panels. The model predicts the
    panel that was excluded from the graph with the HIGHEST cohesion score.
    """
    def __init__(self, num_panels=5, resnet_base_channels=64,
                 pos_embedding_dim=128, gnn_hidden_dim=128, relation_net_mlp_layers=3,
                 relation_net_dropout=0.1, gnn_message_dropout_p=0.1,
                 reasoner_proj_dropout=0.3):

        self.panels_per_problem = num_panels
        self.num_nodes_per_graph = num_panels - 1

        # Create the correct graph template for our P-1 node graphs.
        cohesion_template = create_fully_connected_template(self.num_nodes_per_graph)

        super().__init__(
            # Pass the dynamically created template to the parent.
            template_edge_index=cohesion_template,

            # These are now arbitrary for the parent's init, but required.
            ctx_size=self.num_nodes_per_graph - 1,
            cand_size=1,

            # Pass through all other configuration parameters.
    ...
    def forward(self, x: torch.Tensor, ctx_size=None, cand_size=None) -> torch.Tensor:
        """
        Processes a batch of OOO problems using cohesion scoring. This completely
        overrides the parent's forward pass logic.

        Args:
            imgs (torch.Tensor): Input images. Shape: [B, P, H, W] or [B, P, C, H, W]
            ctx_size, cand_size: Ignored. Kept for signature compatibility.

        Returns:
            torch.Tensor: Cohesion scores. Shape: [B, P]
        """
        if x.dim() == 4:
            B, P_total, H, W = x.shape; C_in = 1
        else: # dim == 5
        ...
        if P_total != self.panels_per_problem:
            raise ValueError(f"Input has {P_total} panels; model expects {self.panels_per_problem}.")

        # 1. Feature Extraction (reusing parent's extractor)
        visual_features_dict = self.extractor(x.view(B * P_total, C_in, H, W))

        # 2. Data Structuring for Cohesion Scoring (complete rework of parent logic)
        all_graphs_nodes_dict = {name: [] for name in self.extractor.fdim_dict.keys()}
        for name, features in visual_features_dict.items():
            fdim = self.extractor.fdim_dict[name]
            structured_features = features.view(B, P_total, fdim)
            for i in range(P_total):
                mask = torch.ones(P_total, dtype=torch.bool, device=x.device)
                mask[i] = False
                graph_nodes = structured_features[:, mask, :] # Shape: [B, P-1, D]
                all_graphs_nodes_dict[name].append(graph_nodes)

        # Final preparation: stack and flatten for GNN
        num_graphs = B * P_total
        for name, graph_list in all_graphs_nodes_dict.items():
            fdim = self.extractor.fdim_dict[name]
            stacked_graphs = torch.stack(graph_list).permute(1, 0, 2, 3) # -> [B, P, P-1, D]
            flat_nodes = stacked_graphs.reshape(-1, fdim) # -> [(B*P*(P-1)), D]
            all_graphs_nodes_dict[name] = flat_nodes

        # 3. Graph Preparation (reusing parent's batching logic)
        batch_indices = torch.arange(
            num_graphs, device=x.device
        ).repeat_interleave(self.num_nodes_per_graph)
        ...
        # 4. Relational Reasoning (reusing parent's reasoner)
        scores = self.reasoner(all_graphs_nodes_dict, batch_indices, edge_index, self.num_nodes_per_graph)

        # Reshape [B*P, 1] -> [B, P]
        return scores.view(B, P_total)
